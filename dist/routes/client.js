...
async function clientRoutes(fastify, options) {
    ...
    fastify.post('/register', async (request, reply) => {
        const { storeName, address, phone, deviceId } = request.body;
        ...
        try {
            ...
            const { data: existingClient, error: getError } = await supabase_1.supabase
                .from('clients')
                .select('*')
            ...
            const { data: newClient, error: insertError } = await supabase_1.supabase
                .from('clients')
                .insert({
